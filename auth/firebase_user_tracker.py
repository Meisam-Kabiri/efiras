            ...
            async with self.connection_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    deleted_daily = await conn.execute(
                        "DELETE FROM daily_usage WHERE user_id = $1", user_id
                    )
                    ...
                    await conn.execute(
                        "DELETE FROM daily_usage WHERE user_id IN (SELECT user_id FROM users WHERE email = $1)",
                        email,
                    )
                    await conn.execute(
                        "DELETE FROM query_logs WHERE user_id IN (SELECT user_id FROM users WHERE email = $1)",
                        email,
                    )
                    await conn.execute("DELETE FROM users WHERE email = $1", email)

                    self.logger.info(
                        f"✅ Deleted user account: {email} (user_id: {user_id})"
                    )
                    return True
