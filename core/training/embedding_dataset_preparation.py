...
def is_valid_sentence(sentence):
    ...
    return (
        word_count >= 10  # Skip very short sentences
        and word_count <= 100  # Skip overly long sentences
        and not sentence.strip().endswith(":")  # Skip header-like sentences
        and not re.match(r"^\d+\.\s*$", sentence.strip())  # Skip numbered items only
    )
...
class AdaptiveMNRLDatasetBuilder:
    ...
    def extract_lowest_level_key(self, header: str) -> str:
        ...
        levels = [
            extract_header_identifier(lev)
            for lev in levels
            if extract_header_identifier(lev) is not None
        ]
        if levels:
    ...
    def group_by_lowest_level(self, chunks: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        print(
            f"Grouped into {len(level_groups)} distinct hierarchy levels (no overlap)"
        )
    ...
    def create_sentence_level_pairs(
        self, chunks: List[Dict], group_key: str = ""
    ) -> List[Dict]:
        all_pairs = {}
        for chunk in chunks:
            content = chunk["text"]
    ...
    def create_contrastive_positive_negative_pairs(self, chunks):
        ...
        random_index_pair_for_negative, x_rest = extract_pairs_with_min_diff(
            list(range(0, len(flattened_pairs))), 20
        )
        contrastive_negative_pairs = [
            {
                "anchor": flattened_pairs[i[0]],
                "positive": flattened_pairs[i[1]],
                "label": 0,
            }
            for i in random_index_pair_for_negative
        ]
    ...
    def create_fixed_size_batches(
        self, all_pairs: Dict[str, List[Dict]], batch_size: int = 8
    ) -> List[List[Dict]]:
        """
        ...
        sorted_groups = sorted(
            all_pairs.keys(), key=lambda x: (self.determine_hierarchy_level(x), x)
        )

        # Create ordered dict to maintain hierarchy order
        ordered_grouped_pairs = {
            group: all_pairs[group].copy()
            ...
            if len(batch) == batch_size:
                ...
                if len(batches) % 20 == 0:
                    remaining_groups = len(
                        [k for k, v in ordered_grouped_pairs.items() if v]
                    )
                    print(
                        f"Created {len(batches)} batches, {remaining_groups} groups still active"
                    )
            else:
                break

        print(
            f"✅ Created {len(batches)} fixed-size batches maintaining hierarchy order"
        )
        return batches

    def generate_fixed_size_mnrl_batches(
        self, chunks: List[Dict], batch_size: int = 8
        ...
        for i, chunk in enumerate(chunks):
            if isinstance(chunk, dict) and len(chunk.get("text", "").strip()) >= 30:
                ...
                processed_chunk["financial_concepts"] = self.extract_financial_concepts(
                    chunk.get("text", "")
                )
                processed_chunk["content_length"] = len(chunk.get("text", ""))
                processed_chunk["regulation_type"] = self.identify_regulation_type(
                    chunk.get("text", "")
                )
        ...
        if len(blocks) < 20:
            print(
                "Warning: Very few blocks found. Need at least 20 blocks for meaningful pairs."
            )
            return []
        ...
        for batch in fixed_batches:
            mnrl_batch = []
            for pair in batch:
                mnrl_batch.append({"anchor": pair[0], "positive": pair[1]})
            mnrl_batches.append(mnrl_batch)

        final_pairs = sum(len(batch) for batch in mnrl_batches)
        print(
            f"✅ Fixed-size MNRL dataset: {len(mnrl_batches)} batches with {final_pairs} total pairs"
        )
