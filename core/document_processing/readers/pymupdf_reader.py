...
class PyMuPDFProcessor:
    ...
    def extract_blocks(self, file_path: Union[str, Path] = "") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            normal_font_size, normal_color = (
                self.find_normal_font_size_and_color_whole_doc(doc)
            )
            ...
            for page_num in range(doc.page_count):
                ...
                for i, block in enumerate(dict_data["blocks"]):
                    # Check if this block has bold text
                    is_diff_format = False
                    if (
                        i < len(dict_data["blocks"])
                        and "lines" in dict_data["blocks"][i]
                    ):
                        for line in dict_data["blocks"][i]["lines"]:
                            for span in line["spans"]:
                                if self.has_different_format(
                                    span, normal_font_size, normal_color
                                ):  # Bold flag
                                    is_diff_format = True
                                    break
                            if is_diff_format:
    ...
    def _save_extracted_blocks(
        ...
        """
        saving_path = (
            f"data/data_processed/{output['filename_without_ext']}_raw_blocks.json"
        )
    ...
    def has_different_format(
        self, span, normal_font_size, normal_color, size_threshold=1, color_tolerance=10
    ):
...
# import fitz
# path = "data/raw/regulatory_documents/eu/Basel_III.pdf"
# doc = fitz.open(path)
...
#     result = processor.extract_blocks("data/raw/regulatory_documents/eu/Basel_III.pdf")
#     result = processor.extract_blocks("data/raw/regulatory_documents/lu/Lux_cssf18_698eng.pdf")
# else:
...
# doc = fitz.open("data/raw/regulatory_documents/eu/Basel_III.pdf")
# bold_blocks = []
