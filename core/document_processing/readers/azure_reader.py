...
class AzureDocumentProcessor(DocumentProcessor):
        ...
        try:
            ...
            for page_idx, page in enumerate(result.pages):
                ...
                for line in page.lines:
                    ...
                    blocks.append(
                        {"page": page_idx + 1, "bbox": bbox, "text": line.content}
                    )
            ...
            logger.info(
                f"Extracted {len(blocks)} blocks from {filename} using Azure Document Intelligence"
            )
    ...
    def _save_extracted_blocks(self, output: Dict[str, Any]) -> None:
        ...
        saving_path = f"data/data_processed/{output['filename_without_ext']}_raw_blocks_azure.json"
