# from core.document_processing.readers.processors.base import DocumentProcessor, ProcessorConfig, ProcessorType
import json
import logging
import re
...
class BlockProcessor:
    ...
    def __init__(
        self, pdf_content: List[Dict[str, Any]] = None, enable_save: bool = True
    ):
        self.pdf_content = pdf_content
        self.if_save = enable_save

    def process_blocks(
        self, pdf_content: List[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not toc:
            ...
            is_toc = True
        blocks = [
            block for block in blocks if not block.get("is_toc", False)
        ]  # remove the toc from blocks

        # [print(f"{key}: {value}") for d in toc for key, value in d.items()]

        blocks = self._extract_headers(blocks)
        blocks = self._clean_text(blocks)
        blocks = [
            block
            for block in blocks
    ...
    def merge_short_blocks_with_next(
        self, blocks: List[Dict[str, Any]], min_length: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _merge_blocks_with_colon_pattern(
        ...
        while i < len(blocks):
            ...
            for level, pattern in patterns.items():
                ...
                if match:
                    ...
                    elif i + 1 < len(blocks) and self._is_title_block(
                        blocks[i + 1]["text"].strip(), blocks[i + 1]
                    ):
                    ...
                    block["header_identifier"] = " > ".join(
                        [
                            self._extract_header_identifier(v)
                            for v in current_headers.values()
                        ]
                    )
            ...
            if not header_found:
                ...
                block["headers"] = (
                    " > ".join(current_headers.values()) if current_headers else ""
                )
                block["header_identifier"] = " > ".join(
                    [
                        self._extract_header_identifier(v)
    ...
    def _save_processed_blocks(
        ...
        """
        saving_path = (
            f"data/data_processed/{filename_without_ext}_processed_blocks.json"
        )
    ...
    def _find_toc_match(
        self, header: str, toc: List[Dict[str, Any]]
    ...
    def _enrich_blocks_with_titles(
        self, blocks: List[Dict[str, Any]], toc: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def reattach_split_paragraphs_across_pages(
        self, blocks: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        def should_merge_blocks(
            current: Dict[str, Any], next_block: Dict[str, Any]
        ) -> bool:
...
#     file_path = Path('data/data_processed/Lux_cssf18_698eng_raw_blocks.json')
#     # file_path = Path("data/data_processed/Basel_III_raw_blocks.json")
