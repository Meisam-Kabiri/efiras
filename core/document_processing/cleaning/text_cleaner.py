...
class RegulationCleaner:
    ...
    def _remove_table_of_contents(self, full_doc_text):
        ...
        toc_header_pattern = (
            r"TABLE OF CONTENTS"  # Case-sensitive based on your example
        )
        ...
        if not toc_header_match:
            print(
                "Warning: 'TABLE OF CONTENTS' header not found. Returning original text."
            )
        ...
        if last_toc_line_end_index == -1:
            print(
                "Warning: No TOC-like lines found after 'TABLE OF CONTENTS' header. Returning original text."
            )
            return full_doc_text
        ...
        for marker_pattern in content_start_markers:
            ...
            match = re.search(
                marker_pattern, full_doc_text[last_toc_line_end_index:], re.IGNORECASE
            )
            if match:
        ...
        if actual_content_start_index != -1:
            print(
                f"Removed TOC from index {toc_header_match.start()} to {actual_content_start_index}"
            )
            return full_doc_text[actual_content_start_index:]
        else:
            print(
                "Warning: Could not reliably identify the start of actual content after the TOC. Attempting to trim after the last TOC line."
            )
