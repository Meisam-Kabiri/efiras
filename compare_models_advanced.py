...
class EmbeddingModelComparator:
    ...
    def evaluate_relevance(self, results: List[Tuple[Dict, float]], expected_sections: List[str]) -> Dict[str, float]:
        ...
        for expected in expected_sections:
            ...
            section_match = re.search(r'(sub-)?section\s+(\d+(?:\.\d+)*)|part\s+([IVX]+)', expected.lower())
            if section_match:
    ...
    def display_results(self, question: Dict, original_results: List, fine_tuned_results: List, 
        ...
        key_metrics = ['binary_section_found', 'content_relevance', 'top_chunk_relevance', 'overall_score']
        ...
        ft_sections = fine_tuned_eval.get('exact_section_matches', 0) if fine_tuned_eval else 0
        ...
        if original_results and fine_tuned_results:
            ...
            if ft_top_sim > orig_top_sim:
                print(f"✅ Fine-tuned model: +{ft_top_sim - orig_top_sim:.4f} higher similarity")
            elif orig_top_sim > ft_top_sim:
                print(f"❌ Original model: +{orig_top_sim - ft_top_sim:.4f} higher similarity")
            else:
        ...
        if fine_tuned_results:
            ...
            self._display_chunk_results(fine_tuned_results[:3], question['expected_sections'])
        
        return winners
    
    def _display_chunk_results(self, results: List[Tuple[Dict, float]], expected_sections: List[str]):
        ...
        for i, (chunk, score) in enumerate(results, 1):
            ...
            relevance_str = " ".join(relevance_indicators) if relevance_indicators else "❌ NOT_RELEVANT"
    ...
    def print_final_summary(self, overall_winners: Dict, detailed_results: List):
        ...
        for model, count in section_detection_performance.items():
            if count > 0:
                ...
                print(f"  {model.replace('_', '-').title()}: {count}/{total_questions} questions ({percentage:.1f}%)")
        
        print(f"\n📈 PERFORMANCE BY CATEGORY:")
        for category, scores in category_performance.items():
            total = sum(scores.values())
            best = max(scores.keys(), key=lambda k: scores[k])
            print(f"  {category}: {best.replace('_', '-').title()} wins ({scores[best]}/{total})")
