...
class AzureKeyVaultManager:
    ...
    def __init__(self, vault_url: Optional[str] = None):
        ...
        try:
            ...
            if self.vault_url:
                ...
                print(
                    "⚠️  Azure Key Vault URL not provided, falling back to environment variables"
                )

        except ImportError:
            print(
                "⚠️  Azure Key Vault libraries not installed. Install with: pip install azure-keyvault-secrets azure-identity"
            )
    ...
    def get_secret(
        ...
        if self.available and self.client:
            try:
                ...
                print(
                    f"⚠️  Failed to retrieve secret '{secret_name}' from Key Vault: {e}"
                )
        ...
        if fallback_env_var:
            ...
            if value:
                print(
                    f"✅ Retrieved secret '{secret_name}' from environment variable '{fallback_env_var}'"
                )
                return value
            else:
                print(
                    f"❌ Secret '{secret_name}' not found in Key Vault or environment variable '{fallback_env_var}'"
                )
    ...
    def get_config_for_rag(self) -> Dict[str, Optional[str]]:
        ...
        """
        config = {
            "azure_openai_endpoint": self.get_secret(
                "azure-openai-endpoint", "AZURE_OPENAI_ENDPOINT"
            ),
            "azure_openai_api_key": self.get_secret(
                "azure-openai-api-key", "AZURE_OPENAI_API_KEY"
            ),
            "openai_api_key": self.get_secret("openai-api-key", "GPT_API_KEY"),
            "azure_document_intelligence_endpoint": self.get_secret(
                "azure-di-endpoint", "AZURE_ENDPOINT"
            ),
            "azure_document_intelligence_key": self.get_secret(
                "azure-di-key", "AZURE_KEY"
            ),
        }
