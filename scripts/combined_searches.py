...
def hybrid_search_simple_comb(
    query, query_embedding, chunks, weights=[0.7, 0.3], top_k=5
):
    # 1. Vector search using existing embeddings
    chunk_embeddings = np.array([chunk["embedding"] for chunk in chunks])
    vector_scores = cosine_similarity([query_embedding], chunk_embeddings)[0]
...
# path = 'data/data_processed/Lux_cssf18_698eng_embeddings_openai_online.json'
# with open(path, 'r') as f:
...
relevant_chunks = asyncio.run(
    search_service.hybrid_search_with_cross_encoder(q, q_embed, top_k=8)
)
for chunk in relevant_chunks:
    print(chunk["chunk_id"], ",", end="", flush=True)
print("\n=========================================================")

relevant_chunks = hybrid_search_rrf(
    q, q_embed, embeddings_data["embeddings"], top_k=20, k=60
)
for chunk in relevant_chunks:
    print(chunk["chunk_id"], ",", end="", flush=True)
print()
