...
def create_local_embeddings_for_all_pdf_in_directory(path:str):
    ...
    for file in files:
        ...
        embd_dir = Path("data/data_processed")
        ...
        # Create Embedding for the file: 
          ...
          reader = PyMuPDFProcessor(enable_save = True)
          raw_blocks = reader.extract_blocks(file)
          
          processor = BlockProcessor(raw_blocks, enable_save = True)
          processed_blocks = processor.process_blocks()
          
          chunker = RegulatoryChunkingSystem(processed_blocks, enable_save = True)
...
def create_embeddings_and_indexes(): 
    path = "data/raw/regulatory_documents/eu"
    create_local_embeddings_for_all_pdf_in_directory(path)
    create_indexes ()
# delete_chunk_block_files_pathlib("data/data_processed")
