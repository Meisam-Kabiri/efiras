...
def generate_smart_questions(chunks):
    ...
    for line in lines:
        ...
        if line.startswith("ID:"):
            ...
            result.append({"chunk_id": current_id, "question": question})
            current_id = None
...
def process_chunks_in_batches(chunks, batch_size=15):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        print(
            f"Processing batch {batch_num}/{total_batches}: chunks {i+1} to {min(i+batch_size, len(chunks))}"
        )
        ...
        try:
            ...
            print(
                f"✅ Batch {batch_num} completed: {len(batch_results)} questions generated"
            )
