...
def check_gpu_availability():
    ...
    if torch.cuda.is_available():
        ...
        print(
            f"⚡ CUDA Cores: {device_props.multi_processor_count * 128}"
        )  # Approximate for RTX series
...
def main():
    ...
    if choice in ["1", "3"]:
        ...
        for file_path in processed_files:
            if Path(file_path).exists():
                ...
                dataset = builder.generate_fixed_size_mnrl_batches(
                ...
                if dataset:
                    ...
                    output_path = (
                        f"training_data/mnrl_fixed_batch_dataset_{doc_name}.json"
                    )
                    ...
                    print(
                        f"Generated {len(dataset)} diverse batches with {total_pairs} total pairs"
                    )
                break
    ...
    if choice in ["2", "3"]:
        ...
        if not isinstance(all_batches[0], list):
            raise ValueError(
                "Expected diverse batch format (List[List[Dict]]) - use fine_tune() for flat datasets"
            )
